...
- 과제하면서 느낀점
```
1) 모르는부분을 알아냈습니다 : NOSQL 잘 모르는데 특히
  - MongoDB 에서  : 스토리지엔진 >> WiredTiger 스토리지 엔진 >> LSM 트리 >> 아 끝이없네..
  - Redis 에서 : 무지성으로 그냥 토큰만 넣어놨었는데, 아 PK 가 하나뿐이구나. 진짜 맵이랑 똑같네, 그럼 다른게 뭐지...? 굇수형님들은 C/ RUST 같은거 수정도 하고 직접구현도 하시던데 이런거 해보면 뭔가 의미가 있지 않을까? >> 아 끝이없네..
  - CAP Theorem : 이런용어를 처음들어봤는데.. 오 상당히.. 멋있음요..
  
2) 인덱스 관련
  - 이게 PK 에 걸리는건지, 컬럼에 걸리는건지, 테이블에 걸리는건지 혼돈파티
  - 이게 RDBMS 라면 응당 그래야하는 표준인건지, MYSQL 구현체만의 특징인건지 모름 >> 모르는건 은근슬쩍 애매하게 썻는데 ..
    - 방어적 글쓰기(까임방지) 같은걸 좀 줄이고 좀더 공부해서 정리해서 쓰고싶은데 >> 아 끝이없네.. 
    
3) 안쓴거(못쓴거일수도)
- 트랜잭션도 opti/pasi, isol level, ACID 이런거 추가해야할꺼같은데.. ㅠ
- 조인 : 조인은 뭔가.. 뭔가 더 있을꺼같은데.. 그냥 쿼리 짜면 되는거 말고 뭐가 더 있는지 모르겠음.. 
  - 오라클에만 있는 Hash Join 이런걸 추가해야하나..
