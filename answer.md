...
3. MySQL에서 조인(join)의 역할은 무엇인가요? 다양한 join의 방식에 대해 설명해주세요. // 다시

join은 특정 조건에 따라서 서로 다른 테이블을 묶어서 하나의 결과 테이블로 나타내는 방식이다.

- inner join: 특정 조건이 테이블이 공통으로 가지고 있는 값(외래키로 연결된)에 관한 조건으로 테이블을 묶는 방식이다.
- outer join: 특정한 테이블만이 가지고 있는 값에 관한 조건으로 테이블을 묶는 방식이다.

4. MySQL에서 인덱스(index)란 무엇인가요? // 다시
